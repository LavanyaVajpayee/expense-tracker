//SELF: TO MAKE THE UI RESPONSIVE
import { Dimensions, PixelRatio } from 'react-native';
//destructuring and adding new alias as SCREEN_WIDTH and SCREEN_HEIGHT
...
export const scale = (size: number) => 
    Math.round(
        PixelRatio.roundToNearestPixel(//to prevent blurring on the border
...
export const verticalScale = (size: number) => 
    Math.round(
        PixelRatio.roundToNearestPixel(
            (shortDimension/guidelineBaseHeight)*(size as number)
        )
    );
